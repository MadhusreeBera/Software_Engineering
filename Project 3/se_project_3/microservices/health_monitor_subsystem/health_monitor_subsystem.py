...
def send_email(subject,msg, admin_email):
    ...
    try:
        ...
        logger.log(message="email sent", level='info')

    except Exception as e:
        print('Error:', e)
        logger.log(message="Failed to send email", level='error')
...
def get_all_services():
    db = get_db()
    service_collection = db["service_registry"]
    services = service_collection.find({"deleted": False})
        ...
        for service in services:
            ...
            try:
                ...
                if not instance_health_record:
                    ...
                    subject = "ALERT!!! Microservice Down"
                    ...
                    #update deleted status in service registry db
                    service_collection = db["service_registry"]
                    service_collection.update_one({"id": service['id']}, {"$set": {"deleted": True}})
