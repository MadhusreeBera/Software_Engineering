...
create cached table T_BOOK ( BOK_ID_C varchar(36) not null, BOK_TITLE_C varchar(255) not null, BOK_SUBTITLE_C varchar(255), BOK_AUTHOR_C varchar(255) not null, BOK_GENRE_C varchar(255) not null, BOK_DESCRIPTION_C varchar(4000), BOK_ISBN10_C varchar(10), BOK_ISBN13_C varchar(13), BOK_PAGECOUNT_N integer, BOK_LANGUAGE_C varchar(2), BOK_PUBLISHDATE_D datetime, primary key (BOK_ID_C) );
...
create cached table T_TAG ( TAG_ID_C varchar(36) not null, TAG_IDUSER_C varchar(36) not null, TAG_NAME_C varchar(36) not null, TAG_COLOR_C varchar(7) default '#3a87ad' not null, TAG_CREATEDATE_D datetime, TAG_DELETEDATE_D datetime, TAG_ISPUBLIC_C bit not null, primary key (TAG_ID_C) );
...
create cached table T_USER_BOOK_RATING ( UBR_ID_C varchar(36) not null, UBR_USERID_C varchar(36) not null, UBR_BOOKID_C varchar(36) not null, UBR_RATING_C integer not null, primary key (UBR_ID_C) );
--create cached table T_USER_BOOK_RATING ( UBR_USERID_C varchar(36) not null, UBR_BOOKID_C varchar(36) not null, UBR_RATING_C integer not null, primary key (UBR_USERID_C, UBR_BOOKID_C) );
--alter table T_USER_BOOK_RATING add constraint FK_UBR_IDUSER_C foreign key (UBR_USERID_C) references T_USER (USE_ID_C) on delete restrict on update restrict;
--alter table T_USER_BOOK_RATING add constraint FK_UBR_IDBOOK_C foreign key (UBR_BOOKID_C) references T_BOOK (BOK_ID_C) on delete restrict on update restrict;
