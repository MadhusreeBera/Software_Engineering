...
App.controller('Tag', function($scope, $state, Restangular) {
  ...
  $scope.isPublicChange = function(tag){
    console.log(tag);
    if(tag.isPublic){
      Restangular.one('tag', tag.id).post('makePublic', tag).then(function () {
        console.log('makePublic');
      });
    }
    else{
      Restangular.one('tag', tag.id).post('makePrivate', tag).then(function () {
